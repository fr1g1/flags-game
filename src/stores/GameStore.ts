...
export class GameStore {
    ...
    start = action(() => {
        this.flagStore.fetch()
            .then(flagsInfo => {
                const data = flagsInfo.map(({ en, name }) => ({ en, name }))
    ...
    check = action((name: string) => {
        if (name === this._currentFlag.get()?.name) {
        ...
        this._flags.replace(this.flags.filter(flag => flag.name !== name))
    })
